...
//! tutorial module prevents pilot diagnostics, QD replay, and exported
//! coordinates from using different tessellations.
...
mod tests {
    ...
    fn publication_and_smoke_layouts_match_fcmaes_archive_indices() {
        for capacity in [60, 120] {
            ...
            let archive = Archive::try_new(10, &lower, &upper, capacity, 0, &mut rng).unwrap();
            for y in 0..101 {
                for x in 0..101 {
                    let descriptor = [
